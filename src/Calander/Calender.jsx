...
const Calender = () => {
  ...
  return (
    ...
    <div className="flex w-[100%]  justify-between px-6 py-4 mb-3 border bg-[#8d99ae]">
    ...
    <div className="lg:w-[85%] h-screen md:w-[90%] sm:[95%] m-auto">
        ...
        {calender.map((week,i)=><div key={i} className='w-full grid grid-cols-7   md:px-8 px-4'>
            {week.map((day,i)=><div key={i} className=' border lg:h-20 md:h-16 h-12 flex items-center justify-center' style={{backgroundColor:`${today(day)?"blue":""}`, color:`${today(day)?"white":"black"}`}}>
